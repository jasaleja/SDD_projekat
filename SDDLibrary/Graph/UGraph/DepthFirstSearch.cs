...
namespace SDDLibrary.Graph.UGraph
{
         ...
         where TEdge : IEdge<TVertex>
    {
        ...
        private IUndirectedGraph<TVertex, TEdge> graph;
        ...
        public void Search(IUndirectedGraph<TVertex, TEdge> graph, params TVertex[] roots)
        {
            Search(graph, (IEnumerable<TVertex>)roots);
        }

        public void Search(IUndirectedGraph<TVertex, TEdge> graph, IEnumerable<TVertex> roots)
        {
            Initialize(graph);

            if (roots == null || roots.Count() == 0)
                throw new ArgumentException("Root nodes must be supplied to dfs search in undirected graph.");
